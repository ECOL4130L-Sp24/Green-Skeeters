                         ...
                         ifelse(month %in% c(09,10,11), "fall",
                                ifelse(month %in% c(12,01,02), "winter",
                                        ifelse(month %in% c(03,04,05), "spring",
                                                "NA"))))) %>% 
  dplyr::select(siteID, season, testResult_mos, avg_temp) %>% 
  summarize(mean_temp = mean(avg_temp)) %>% 
  dplyr::group_by(siteID, season, testResult_mos, mean_temp) %>% 
  summarize(total = n()) %>% 
...
prop_positive_lm <- lm(prop_pos ~ temperature, data = temp_mos_season_prop)
